...
type MapKey = any[] | object | string | boolean | number
...
type asMapParamOf<K, V> = K extends string
  ? Record<string, V> | Array<{ key: K, value: V }>
  : K extends number
    ? Record<number, V> | Array<{ key: K, value: V }>
    : Array<{ key: K, value: V }>
...
function asMap<K extends MapKey, V> (value: asMapParamOf<K, V>): MMap<K, V> {
  ...
  if (Array.isArray(value)) {
    ...
    const vObject = value as Record<string, V>
