...
const isEvmChainFunded = async ({ evmChainConfig, evmWallet }: IEvmChainConfigAndEvmWallet): Promise<boolean> => {
    let isFunded = true;
    try {
        const currentBalance = await getCurrentEvmBalance({ evmChainConfig, evmWallet });
        const remainingRaw = BigInt(evmChainConfig.intialFund) - BigInt(currentBalance);
