...
const isEvmChainFunded = async ({
    ...
    try {
        ...
        if (currentBalance < BigInt(evmChainConfig.intialFund)) {
            isFunded = false;
            console.info(
                `Current balance: ${ethers.formatEther(currentBalance)}; Fund chain your wallet ${evmWallet.address} on ${evmChainConfig.chain} with ${remainingFund} ${evmChainConfig.nativeCoinSymbol}.`,
            );
        }
