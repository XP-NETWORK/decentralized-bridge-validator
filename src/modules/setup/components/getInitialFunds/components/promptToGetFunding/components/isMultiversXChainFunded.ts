...
const isMultiversXChainFunded = async ({
    ...
    try {
        ...
        if (currentBalance < BigInt(multiversXChainConfig.intialFund)) {
            isFunded = false;
            console.info(
                `Current balance: ${ethers.formatEther(currentBalance)}; Fund chain your wallet ${multiversXWallet.userWallet.bech32} on ${multiversXChainConfig.chain} with ${remainingFund} ${multiversXChainConfig.nativeCoinSymbol}.`,
            );
        }
