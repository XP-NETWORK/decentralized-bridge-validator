...
const isSecretChainFunded = async ({
    ...
    try {
        ...
        if (currentBalance < BigInt(secretChainConfig.intialFund)) {
            isFunded = false;
            console.info(
                `Current balance: ${formatSecret(currentBalance)}; Fund chain your wallet ${pubkeyToAddress(Buffer.from(secretWallet.publicKey, 'hex'))} on ${secretChainConfig.chain} with ${remainingFund} ${secretChainConfig.nativeCoinSymbol}.`,
            );
        }
