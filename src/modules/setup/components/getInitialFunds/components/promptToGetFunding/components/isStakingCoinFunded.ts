...
const isStakingCoinFunded = async ({
    ...
    try {
        ...
        if (stakingCoinCurrentBalance < BigInt(stakingChainConfig.intialFund)) {
            isFunded = false;
            console.info(
                `Current balance: ${ethers.formatEther(stakingCoinCurrentBalance)}; Fund chain your wallet ${evmWallet.address} on ${stakingChainConfig.chain} with ${remainingStakingCoinFund} ${stakingChainConfig.coinSymbol} [ Coin address ${stakingChainConfig.coinAddress}].`,
            );
        }
