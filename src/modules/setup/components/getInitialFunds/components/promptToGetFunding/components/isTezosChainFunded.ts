...
const isTezosChainFunded = async ({ tezosChainConfig, tezosWallet }: ITezosChainConfigAndTezosWallet): Promise<boolean> => {
    let isFunded = true;
    try {
        const currentBalance = await getCurrentTezosBalance({ tezosChainConfig, tezosWallet });
        const remainingRaw = BigInt(tezosChainConfig.intialFund) - BigInt(currentBalance);
        ...
        if (currentBalance < BigInt(tezosChainConfig.intialFund)) {
            isFunded = false
            console.info(`Current balance: ${formatTezos(currentBalance)}; Fund chain your wallet ${addressToFund} on ${tezosChainConfig.chain} with ${remainingFund} ${tezosChainConfig.nativeCoinSymbol}.`);
        }
