...
import { type ITonChainConfigAndTonWallet } from '@src/types'
...
const isTonChainFunded = async ({
  ...
  try {
    ...
    if (currentBalance < BigInt(tonChainConfig.intialFund)) {
      isFunded = false
      console.info(
                `Current balance: ${tonweb.utils.fromNano(
                    currentBalance
                )}; Fund chain your wallet ${await wallet.getAddress()} on ${
                    tonChainConfig.chain
                } with ${remainingFund} ${tonChainConfig.nativeCoinSymbol}.`
      )
    }
