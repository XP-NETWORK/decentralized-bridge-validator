...
const isTonChainFunded = async ({ tonChainConfig, tonWallet }: ITonChainConfigAndTonWallet): Promise<boolean> => {
    let isFunded = true;
    try {
        ...
        if (currentBalance < BigInt(tonChainConfig.intialFund)) {
            isFunded = false
            console.info(`Current balance: ${tonweb.utils.fromNano(currentBalance)}; Fund chain your wallet ${await wallet.getAddress()} on ${tonChainConfig.chain} with ${remainingFund} ${tonChainConfig.nativeCoinSymbol}.`);
        }
