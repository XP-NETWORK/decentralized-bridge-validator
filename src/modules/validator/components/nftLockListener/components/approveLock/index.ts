...
const approveLock = async ({ nftTransferDetailsObject, wallets, destinationChainObject, storageContract, txChain }: IApproveLock) => {
    ...
    if (destinationChainObject.chainType === "evm") {
        ...
        ({ publicAddress, signature } = await getHederaSignedNftDetails({ nftTransferDetailsObject, evmWallet: wallets.evmWallet }))
    }
        ...
        ({ publicAddress, signature } = await getTezosSignedNftDetails({ nftTransferDetailsObject, tezosWallet: wallets.tezosWallet }))
    }
