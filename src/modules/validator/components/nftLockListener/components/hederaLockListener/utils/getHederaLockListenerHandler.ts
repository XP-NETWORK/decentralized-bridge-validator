...
import { IEvmLockListener, IHederaLockListener } from "../../../types";
...
const getHederaLockListenerHandler = ({ config, hederaChainConfig, wallets }: IHederaLockListener) => {


    const bridgeContract = getHederaBridgeContract({  hederaChainConfig, evmWallet: wallets.evmWallet });
    ...
    const handleLog = async ({ log }: { log: LogEntry }) => {
        ...
        if (sourceChainObject) {
            ...
            await approveLock({ nftTransferDetailsObject, wallets, storageContract, txChain: hederaChainConfig.chain, destinationChainObject })
        }
    }
