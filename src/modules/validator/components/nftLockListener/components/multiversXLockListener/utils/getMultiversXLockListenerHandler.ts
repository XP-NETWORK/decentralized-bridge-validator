...
import { type IMultiversXLockListener } from '../types'
...
import { type IHandleLog } from '@src/modules/validator/utils/multiversXContractListener/components/types'
import { MultiversXTransactions } from '@src/db/entity/MultiversXTransactions'

const getMultiversXLockListenerHandler = ({
  config,
  ...
  const handleLog: IHandleLog = async ({
    log,
    ...
    if (sourceChainObject) {
      ...
      await approveLock({
        ...
        wallets,
        storageContract,
        txChain: multiversXChainConfig.chain,
