...
const nftLockListener = async ({ config, wallets }: IConfigAndWallets) => {
    ...
    for (const chainConfig of config.bridgeChains) {
        if (chainConfig.chainType === 'evm') {
            ...
            const jobData: IHederaLockListener = {hederaChainConfig: chainConfig, config, wallets};
            const jobName: string = `hederaLockedEventListener_${chainConfig.chain}`
            await createJobWithWorker<IHederaLockListener>({ jobData, jobName, jobFunction: hederaLockListener })
        }
