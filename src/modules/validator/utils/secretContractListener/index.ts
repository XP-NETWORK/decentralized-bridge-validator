...
async function secretContractListener(
    { contractAddress,
        ...
        chain,
        eventId,
        handleLog }: ISecretContractListener): Promise<void> {
    ...
    for (const log of logs) {
        const logToFind = log.jsonLog[0].events.find(item => item.type === "wasm").attributes.find(item => item.key === eventId);
        if (logToFind)
            handleLogPromises.push(handleLog({ log: logToFind.value, hash: log.transactionHash }));
    }
