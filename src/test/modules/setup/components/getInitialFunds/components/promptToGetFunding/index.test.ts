    ...
    const testCases = [
        {
            ...
            fundedChains: [...mockBridgeConfig.bridgeChains.map(item => item.chain), mockBridgeConfig.stakingConfig.chain, mockBridgeConfig.storageConfig.chain],
            expected: true,
        },
        {
            description: 'should return false one bridge chain not funded',
            fundedChains: [...[...mockBridgeConfig.bridgeChains.map(item => item.chain)].slice(1), mockBridgeConfig.stakingConfig.chain, mockBridgeConfig.storageConfig.chain],
            expected: false,
        },
        {
            description: 'should return false no bridge chains are funded',
            fundedChains: [mockBridgeConfig.stakingConfig.chain, mockBridgeConfig.storageConfig.chain],
            expected: false,
        },
        {
            description: 'should return false if staking chain is not funded',
            fundedChains: [...mockBridgeConfig.bridgeChains.map(item => item.chain), mockBridgeConfig.storageConfig.chain],
            expected: false,
        },
        {
            description: 'should return false if storage chain is not funded',
            fundedChains: [...mockBridgeConfig.bridgeChains.map(item => item.chain), mockBridgeConfig.stakingConfig.chain],
            expected: false,
        },
        {
    ...
    testCases.forEach(({
        ...
        it(description, async () => {
            ...
            sinon.stub(utils, 'isStakingCoinFunded').callsFake(({ stakingChainConfig }) => {
                return Promise.resolve(fundedChains.includes(stakingChainConfig.chain)); // Resolve with true if staking coin is funded
            });


            const result = await promptToGetFunding({ config: mockBridgeConfig, wallets: mockWallets });
