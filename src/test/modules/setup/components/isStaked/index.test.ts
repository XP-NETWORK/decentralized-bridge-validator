...
describe('isStaked', () => {
    ...
    const testCases = [
        {
            ...
            expected: true,
            description: 'should return true if stakedAmount is greater than zero',
        },
        {
    ...
    testCases.forEach(({ stakedAmount, expected, description }) => {
        it(description, async () => {
            ...
            getStakingContractStub.returns({...getStakingContractStub[1], ...stakingContractStub});

            const result = await isStaked({ stakingChainConfig: mockBridgeConfig.stakingConfig, evmWallet: mockWallets.evmWallet });
