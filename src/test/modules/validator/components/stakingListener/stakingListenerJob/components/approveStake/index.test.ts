...
describe('approveStake', () => {
  ...
  const testCases = [
    {
      approveStakeStubFunction: sinon
        .stub()
        .resolves({ hash: 'some unique hash' }),
      expectError: false,
    ...
    },
    {
      ...
      expectError: true,
      description:
                'should throw error if error is not of already approved stake'
    }
  ]

  testCases.forEach(
    ({ approveStakeStubFunction, expectError, description }) => {
      it(description, async () => {
        ...
        const storageContract = getStorageContractStub({
          evmChainConfig: mockBridgeConfig[0],
