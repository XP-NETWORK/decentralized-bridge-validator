...
const getEvmMultiNftContract = ({ contractConfig, evmWallet }: IEvmContractConfigAndEvmWallet) : INftContract=> {
    ...
    const erc1155Contract: IExtendedERC1155Royalty = ERC1155Royalty__factory.connect(contractConfig.contractAddress, opWallet);
    ...
    return {
        ...
        symbol: async () => {
            try {
                ...
                console.error("The contract does not have a 'symbol' function.");
                return "";
            }
        },
        royaltyInfo: async (tokenId: bigint) => {
            try {
                const [, royaltyAmount] = await erc1155Contract.royaltyInfo(tokenId, salePriceToGetTotalRoyalityPercentage);
