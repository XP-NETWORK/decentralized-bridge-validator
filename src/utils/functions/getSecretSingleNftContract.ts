...
const getSecretSingleNftContract = ({ rpcURL, chainId, contractAddress }: ISecretContractConfig): INftContract => {
    ...
    return {
        ...
        royaltyInfo: async (tokenId: bigint) => {
            const royalty_info = ((await secretjs.query.compute.queryContract({
                contract_address: contractAddress,
        ...
        tokenURI: async (tokenId: bigint) => {
            const nft_info = ((await secretjs.query.compute.queryContract({
                contract_address: contractAddress,
